...
uint8_t memblock[RECORDSTORE_MEMSIZE];
...
void recordstore_insert(uint8_t * data, uint8_t len) {
	...
	if ( (memblock_pos - memblock) + len + 1 >= RECORDSTORE_MEMSIZE ) {
	...
	printf_P(PSTR(" recordstore %d/%d bytes\n"),memblock_pos-memblock,RECORDSTORE_MEMSIZE);
}

uint8_t * recordstore_dump(uint16_t * len) {
	*len = (uint16_t)(memblock_pos - memblock);
	return memblock;
}
